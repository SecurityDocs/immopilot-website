...
const partners: Record<string, Partner> = {
  "makler-muster": {
    ...
    highlight: "Empfohlen von einem Makler-Partner",
  ...
  "elektro-kaiser": {
    ...
    title: "Elektriker & Dienstleister, Berlin",
    type: "handwerker",
    avatar: "EK",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(FORMSPREE_URL, {
        ...
        body: JSON.stringify({
          ...form,
          _subject: `ImmoPilot Lead via ${partner.name} | ${form.vorname} ${form.nachname}`,
    ...
    return (
      <div className="text-center py-10">
        ...
        <p className="text-slate-500 text-sm max-w-xs mx-auto">
          Wir melden uns innerhalb von 24h. Ihr Bonus-Code{" "}
          <strong className="text-primary-600">{partner.bonusCode}</strong> ist für Sie reserviert.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-primary-50 border border-primary-200 rounded-xl px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-primary-500 mt-0.5">
            Code: <span className="font-bold">{partner.bonusCode}</span> — wird automatisch angewendet
          </p>
        </div>
      </div>
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center px-4">
        ...
        <h1 className="text-2xl font-extrabold text-slate-900 mb-2">Partner-Seite nicht gefunden</h1>
        <p className="text-slate-500 mb-6">Dieser Partner-Link ist nicht mehr aktiv oder wurde falsch eingegeben.</p>
        <a href="/" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary-600 text-white font-semibold text-sm hover:bg-primary-700 transition-colors">
          Zur Startseite
        </a>
      </div>
    </main>
...
export default function PartnerLandingPage({ params }: { params: Promise<{ slug: string }> | { slug: string } }) {
  // Next.js 15/16 kompatibel: params kann Promise sein
  const resolvedParams = params instanceof Promise ? use(params) : params;
  const partner = partners[resolvedParams.slug];
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <a href="/" className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-900 text-base">Immo<span className="text-primary-600">Pilot</span></span>
          </a>
      ...
      <div className="pt-14">
        ...
        <div className="bg-slate-950 text-white py-14 lg:py-20">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
              <div>
                <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-white text-xs font-bold mb-6 ${cfg.color}`}>
                  <TypeIcon size={12} />
                  <span>{partner.highlight}</span>
                </div>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold leading-tight mb-4">
                  {partner.customHeadline.includes("ImmoPilot") ? (
                    <>
                      {partner.customHeadline.split("ImmoPilot")[0]}
                      <span className="text-primary-400">ImmoPilot</span>
                      {partner.customHeadline.split("ImmoPilot")[1]}
                    </>
                  ) : partner.customHeadline}
                </h1>
              ...
              {/* Formular */}
              <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-2xl mt-8 lg:mt-0">
                <h2 className="text-xl font-extrabold text-slate-900 mb-1">Jetzt kostenlos starten</h2>
                <p className="text-sm text-slate-500 mb-6">
                  Empfohlen von <strong>{partner.name}</strong>
                </p>
        ...
        <div className="py-6 border-t border-slate-200 bg-white text-center">
          <p className="text-xs text-slate-400">
            © 2026 ImmoPilot ·{" "}
            <a href="/datenschutz" className="hover:text-slate-600 underline">Datenschutz</a> ·{" "}
            <a href="/impressum" className="hover:text-slate-600 underline">Impressum</a>
          </p>
        </div>
      </div>
    </main>
